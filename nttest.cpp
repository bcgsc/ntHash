...
static const char USAGE_MESSAGE[] = "Usage: " PROGRAM " [OPTION]... QUERY\n"
...
bool
getSeq(std::ifstream& uFile, std::string& line)
{
	bool good = false;
	std::string hline;
	line.clear();
...
void
loadSeq(BloomFilter& myFilter, const string& seq)
{
...
void
loadSeqr(BloomFilter& myFilter, const string& seq)
{
	if (seq.size() < opt::kmerLen)
		return;
...
void
loadSeqm(BloomFilter& myFilter, const string& seq)
{
	if (seq.size() < opt::kmerLen)
		return;
	for (size_t i = 0; i < seq.size() - opt::kmerLen + 1; i++) {
...
void
loadSeqc(BloomFilter& myFilter, const string& seq)
{
	if (seq.size() < opt::kmerLen)
		return;
	for (size_t i = 0; i < seq.size() - opt::kmerLen + 1; i++) {
...
void
loadSeqx(BloomFilter& myFilter, const string& seq)
{
	if (seq.size() < opt::kmerLen)
		return;
	for (size_t i = 0; i < seq.size() - opt::kmerLen + 1; i++) {
...
void
loadBf(BloomFilter& myFilter, const char* faqFile)
{
	getFtype(faqFile);
...
void
querySeqr(BloomFilter& myFilter, const string& seq, size_t& fHit)
{
	if (seq.size() < opt::kmerLen)
		return;
	uint64_t fhVal, rhVal;
...
void
querySeq(BloomFilter& myFilter, const string& seq, size_t& fHit)
{
	if (seq.size() < opt::kmerLen)
		return;
	for (size_t i = 0; i < seq.size() - opt::kmerLen + 1; i++) {
...
void
querySeqm(BloomFilter& myFilter, const string& seq, size_t& fHit)
{
	if (seq.size() < opt::kmerLen)
		return;
	for (size_t i = 0; i < seq.size() - opt::kmerLen + 1; i++) {
...
void
querySeqc(BloomFilter& myFilter, const string& seq, size_t& fHit)
{
	if (seq.size() < opt::kmerLen)
		return;
	for (size_t i = 0; i < seq.size() - opt::kmerLen + 1; i++) {
...
void
querySeqx(BloomFilter& myFilter, const string& seq, size_t& fHit)
{
	if (seq.size() < opt::kmerLen)
		return;
	for (size_t i = 0; i < seq.size() - opt::kmerLen + 1; i++) {
...
void
queryBf(BloomFilter& myFilter, const char* faqFile)
{
	getFtype(faqFile);
	ifstream uFile(faqFile);
...
void
hashSeqb(const string& seq)
{
	for (size_t i = 0; i < seq.length() - opt::kmerLen + 1; i++) {
		if (NTC64(seq.c_str() + i, opt::kmerLen))
			opt::nz++;
	}
}

void
hashSeqr(const string& seq)
{
...
void
hashSeqx(const string& seq)
{
	for (size_t i = 0; i < seq.length() - opt::kmerLen + 1; i++) {
		string kmer = seq.substr(i, opt::kmerLen);
		getCanon(kmer);
		if (XXH64(kmer.c_str(), opt::kmerLen, 0))
			++opt::nz;
	}
}

void
hashSeqc(const string& seq)
{
	for (size_t i = 0; i < seq.length() - opt::kmerLen + 1; i++) {
		string kmer = seq.substr(i, opt::kmerLen);
		getCanon(kmer);
		if (CityHash64(kmer.c_str(), opt::kmerLen))
			++opt::nz;
	}
}

void
hashSeqm(const string& seq)
{
	for (size_t i = 0; i < seq.length() - opt::kmerLen + 1; i++) {
		string kmer = seq.substr(i, opt::kmerLen);
		getCanon(kmer);
		if (MurmurHash64A(kmer.c_str(), opt::kmerLen, 0))
			++opt::nz;
	}
}

void
hashSeqbM(const string& seq)
{
...
void
hashSeqxM(const string& seq)
{
	for (size_t i = 0; i < seq.length() - opt::kmerLen + 1; i++) {
		string kmer = seq.substr(i, opt::kmerLen);
		getCanon(kmer);
		for (unsigned h = 0; h < opt::nhash; h++)
			if (XXH64(kmer.c_str(), opt::kmerLen, h))
				++opt::nz;
	}
}

void
hashSeqcM(const string& seq)
{
	for (size_t i = 0; i < seq.length() - opt::kmerLen + 1; i++) {
		string kmer = seq.substr(i, opt::kmerLen);
		getCanon(kmer);
		for (unsigned h = 0; h < opt::nhash; h++)
			if (CityHash64WithSeed(kmer.c_str(), opt::kmerLen, h))
				++opt::nz;
	}
}

void
hashSeqmM(const string& seq)
{
	for (size_t i = 0; i < seq.length() - opt::kmerLen + 1; i++) {
		string kmer = seq.substr(i, opt::kmerLen);
		getCanon(kmer);
		for (unsigned h = 0; h < opt::nhash; h++)
			if (MurmurHash64A(kmer.c_str(), opt::kmerLen, h))
				++opt::nz;
	}
}

void
nthashBF(const char* geneName, const char* readName)
{
		...
		for (unsigned k = 50; k <= opt::squery; k += 100) {
			...
			for (unsigned i = 1; i < 6; i += 2) {
				...
				BloomFilter myFilter(
				    opt::ibits * opt::ngene * opt::sgene, opt::nhash, opt::kmerLen);
				...
				std::cerr << "load_time=" << setprecision(4) << fixed << omp_get_wtime() - sTime
				          << "\n";
#else
				std::cerr << "load_time=" << setprecision(4)
				...
				std::cerr << "query_time=" << setprecision(4) << fixed << omp_get_wtime() - sTime
				          << "\n";
#else
				std::cerr << "query_time=" << setprecision(4)
				          << (double)(clock() - start) / CLOCKS_PER_SEC << "\n";
#endif
			}
		}
