...
class ssHashIterator
{
  ...
  public:
	...
	uint64_t operator*() const { return m_sVal; }

	/** test equality with another iterator */
	bool operator==(const ssHashIterator& it) const { return m_pos == it.m_pos; }

	/** test inequality with another iterator */
	bool operator!=(const ssHashIterator& it) const { return !(*this == it); }
	...
	static const ssHashIterator end() { return ssHashIterator(); }
